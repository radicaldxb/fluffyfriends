...
export async function GET(request: NextRequest) {
  ...
  const { data: purchaseRows, error: purchaseError } = await supabase
    ...
    .select(
      "id, package, portraits_total, portraits_used, portraits_remaining, created_at, stripe_session_id, payment_intent_id",
    )
    .ilike("email", email)
  ...
  const purchases = (purchaseRows || []).map((p) => ({
    ...
    // Prefer payment_intent_id for order display; fall back to stripe_session_id
    order_id: p.payment_intent_id || p.stripe_session_id || null,
  }))
