...
export async function POST(req: NextRequest) {
  try {
    ...
    const { portrait_id, pet_name, theme, pet_image_url, user_email, showcase_consent } = body

    if (!portrait_id || !pet_name || !theme || !pet_image_url) {
      return NextResponse.json(
        { error: "Missing required fields: portrait_id, pet_name, theme, pet_image_url" },
        { status: 400 },
      )
    }
