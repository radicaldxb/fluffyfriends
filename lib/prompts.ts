/**
 * Theme-specific prompts for Gemini API image generation.
 * 
 ...
 * 
 * These prompts are sent to n8n workflow and used in the GEMINI node.
 */
...
export type ThemePrompt = keyof typeof THEME_PROMPTS

/**
 * Get the prompt for a specific theme.
 * Falls back to fireman prompt if theme is invalid.
 */
export function getPromptForTheme(theme: string): string {
  const normalizedTheme = theme.toLowerCase() as ThemePrompt
